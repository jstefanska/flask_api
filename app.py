...
def get_hashtag(hashtag):
    try:
        ...
        query = sql.SQL("SELECT * FROM public.twitter WHERE hashtag = %s ORDER BY tweet_id DESC LIMIT 10").format()
        cur.execute(query, (hashtag, ))
        tweets_columns = cur.fetchall()

        tweets_data = [{"tweet_id": column[1], "tweet_content": column[0]} for column in tweets_columns]
